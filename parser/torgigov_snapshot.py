...
Собирает ID последних SNAPSHOT_TARGET_COUNT лотов общего списка
(тот же что на главной странице, без обхода категорий — category
не поддерживается как серверный фильтр, см. torgigov_lib.py) → POST /snapshot.
"""
...
def collect_snapshot_ids() -> list[str]:
    """Собирает lot_id с общего списка постранично, пока не наберёт лимит."""
    ids: list[str] = []
    page     = 0
