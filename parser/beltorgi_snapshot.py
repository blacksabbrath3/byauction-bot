"""beltorgi_snapshot.py — первичный слепок beltorgi.by

Фиксирует все лоты, которые сейчас видны на главной странице (дедуп. по
lot_id), как "уже известные" — чтобы daily-парсер не разослал их все как
новые при первом запуске.
"""
...
logging.basicConfig(level=logging.INFO,
    format="%(asctime)s [beltorgi_snapshot] %(levelname)s: %(message)s")
...
def main():
    ...
    log.info(f"Получено {len(ids)} уникальных лотов, сохраняем как известные")
    for lot in lots:
